    ...
    const handleDelete = async() => {
      if (deletePhrase === 'Whoami') {
        try{
          ...
          const response = await axios.post(`${process.env.REACT_APP_BASE_URL}api/passwords/delete`, formData, config)
      ...
      <div className="flex sm:flex-col justify-between mt-4">
        ...
        <div className="w-[27%] sm:w-[60%] sm:mx-[5%] max-h-auto flex flex-col gap-y-6 overflow-hidden">
          <div className="rounded-lg max-h-[310px] bg-white p-6 overflow-y-scroll">
            {/* <div className="flex items-center gap-x-2 font-[500] text-[14px]">Caution<AiOutlineWarning /></div> */}
            <form>
            <div className="flex flex-col">
                <p>Delete all passwords?</p>
                <input onChange={(e)=>setDeletePhrase(e.target.value)} type="text" placeholder="Type Whoami to delete passwords" className="px-[2%] text-[12px] font-[500] w-[100%] focus:outline-none py-[0.7rem] border border-[#E6EDFF] rounded-lg bg-[white]" />
            </div>
            <div className="flex my-[6%]">
                <input
