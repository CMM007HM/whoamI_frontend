...
export const createPassword = createAsyncThunk(
    ...
    async({ fancyName, password, username, categoryId, description }, { getState, rejectWithValue }) => {
        try {
            ...
            const { data } = await axios.post(`${process.env.REACT_APP_BASE_URL}api/passwords/`, { fancyName, password, username, categoryId, description }, config);
...
export const getPasswords = createAsyncThunk(
    ...
    async(arg, { getState, rejectWithValue }) => {
        try {
            ...
            const { data } = await axios.get(`${process.env.REACT_APP_BASE_URL}api/passwords/`, config);
            return data;
        }
        catch (error) {
            if (error.response && error.response.data?.message) {
...
export const updatePassword = createAsyncThunk(
    ...
    async({ id, fancyName, password, username, categoryId, description }, { getState, rejectWithValue }) => {
        try {
            ...
            const { data } = await axios.put(`${process.env.REACT_APP_BASE_URL}api/passwords/${id}`, { fancyName, password, username, categoryId, description }, config);
            // window.location.reload()
            return data;
        } catch (error) {
            if (error.response && error.response.data?.message) {
